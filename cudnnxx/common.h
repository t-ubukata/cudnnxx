...
class Handle {
 public:
  Handle() {
    CUDNNXX_DNN_CHECK(cudnnCreate(&raw_handle_));
  }

  ~Handle() {
    CUDNNXX_DNN_CHECK(cudnnDestroy(raw_handle_));
  }
...
class Tensor {
 public:
  Tensor(cudnnDataType_t dtype, cudnnTensorFormat_t format,
    ...
    CUDNNXX_DNN_CHECK(cudnnSetTensor4dDescriptor(desc_, format, dtype,
                                              n, c, h, w));
  }
  ...
  Tensor(cudnnDataType_t dtype, int n, int c, int h, int w,
         ...
         : dev_mem_(dev_mem) {
    CUDNNXX_DNN_CHECK(cudnnCreateTensorDescriptor(&desc_));
    CUDNNXX_DNN_CHECK(cudnnSetTensor4dDescriptorEx(desc_, dtype,
  ...
  Tensor(cudnnDataType_t dtype, int n_dims, int dims[], int strides[],
    ...
    CUDNNXX_DNN_CHECK(cudnnCreateTensorDescriptor(&desc_));
    CUDNNXX_DNN_CHECK(cudnnSetTensorNdDescriptor(desc_, dtype, n_dims, dims,
                                              strides));
  }
  ...
  Tensor(cudnnTensorFormat_t format, cudnnDataType_t dtype, int n_dims,
    ...
    CUDNNXX_DNN_CHECK(cudnnCreateTensorDescriptor(&desc_));
    CUDNNXX_DNN_CHECK(cudnnSetTensorNdDescriptorEx(desc_, format, dtype, n_dims,
                                                dims));
  }
  ...
  ~Tensor() {
    CUDNNXX_DNN_CHECK(cudnnDestroyTensorDescriptor(desc_));
  }
...
class Filter {
 public:
  Filter(cudnnDataType_t dtype, cudnnTensorFormat_t format,
    ...
    CUDNNXX_DNN_CHECK(cudnnSetFilter4dDescriptor(desc_, dtype, format,
                                              k, c, h, w));
  }
  ...
  Filter(cudnnDataType_t dtype, cudnnTensorFormat_t format, int n_dims,
        int dims[], T* dev_mem) : dev_mem_(dev_mem) {
    CUDNNXX_DNN_CHECK(cudnnCreateFilterDescriptor(&desc_));
    CUDNNXX_DNN_CHECK(cudnnSetFilterNdDescriptor(desc_, dtype, format, n_dims,
                                              dims));
  }
  ...
  ~Filter() {
    CUDNNXX_DNN_CHECK(cudnnDestroyFilterDescriptor(desc_));
  }
